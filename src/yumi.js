...
var properlyConfigured = function() {
    if (!config) {
        return false;
    }

    if (!config.yumi || !config.bamboo || !config.hipchat) {
        return false;
    }

    // Yumi configuration.
    if (!config.yumi.dbUrl) {
        return false;
    }

    // Bambo configuration.
    if (!config.bamboo.domain || !config.bamboo.username || !config.bamboo.password || !config.bamboo.project) {
        return false;
    }

    // Hipchat configuration.
    if (!config.hipchat.apiKey || !config.hipchat.room) {
        return false;
    }
...
if (properlyConfigured()) {
    ...
    console.log('Yumi is not properly configured! Please review your config.json file!');
}